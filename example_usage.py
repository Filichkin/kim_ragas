...
def example_custom_config():
    ...
    custom_config = create_custom_config(
        ...
        embedding=EmbeddingConfig(
            model_name="all-mpnet-base-v2", similarity_threshold=0.8
        ),
    ...
    logger.info(
        f"Стратегия разбиения: {custom_config.chunking.strategy.value}"
    )
    logger.info(f"Модель эмбедингов: {custom_config.embedding.model_name}")
    logger.info(
        f"Количество сценариев: {custom_config.scenario.num_scenarios}"
    )
...
def example_different_chunking_strategies():
    ...
    for strategy in strategies:
        ...
        try:
            generator = RAGASDataGenerator(
                chunking_strategy=strategy, embedding_model="all-MiniLM-L6-v2"
            )
            ...
            logger.info(
                f"Датасет для {strategy.value} сохранен в {output_path}"
            )

        except Exception as e:
            logger.error(
                f"Ошибка при использовании стратегии {strategy.value}: {e}"
            )
...
def example_question_analysis():
    ...
    logger.info(
        "  - Single-hop: вопросы, требующие информацию из одного источника"
    )
    logger.info(
        "  - Multi-hop: вопросы, требующие информацию из нескольких источников"
    )
...
def example_scenario_distribution():
    ...
    distributions = {
        "balanced": {"simple": 0.33, "medium": 0.34, "complex": 0.33},
        "beginner_friendly": {"simple": 0.6, "medium": 0.3, "complex": 0.1},
        "expert_level": {"simple": 0.1, "medium": 0.3, "complex": 0.6},
        "mixed": {"simple": 0.4, "medium": 0.4, "complex": 0.2},
    }
...
def example_batch_processing():
    ...
    for folder_path in document_folders:
        ...
        logger.info(
            f"Обработка {len(document_paths)} документов в {folder_path}"
        )
